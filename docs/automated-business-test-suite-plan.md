...
.\scripts\windows\installed-restart-persistence-smoke.ps1 -ExpectedVersion 0.5.20 -Port 8796
...
| Fresh installed app first-run smoke | Pass for current installed `0.5.20` NSIS state; `0.5.21` install pending | Latest installed-state verifier at 2026-07-16 16:13-16:14 +08:00 ran `run-installed-app-smoke.ps1`, a one-command wrapper for the current Program Files app. It verified installed `0.5.20` NSIS shape, one MergePilot uninstall entry, expected `uninstall.exe`, legacy `C:\Program Files\CICD-Agent` absent, old publisher shortcut absent, current shortcut present, Azure OpenAI `gpt-4o`, authenticated user/avatar, restart persistence, and unexpected-port safety. Wrapper output reported `packageProbe.exitCode: 0`, `persistence.exitCode: 0`, `safety.exitCode: 0`, and `openAfterRun: []`. Strict MSI desktop parity remains separate because the installed desktop is the NSIS bundle shape and `uninstall.exe` is present. The newly built local `0.5.21` MSI/NSIS package has not been installed system-wide yet. Computer Use bootstrap/app discovery/accessibility proof from 09:40 still passes for the native MergePilot window, but activation/click proof remains unreliable. Latest record: `docs/live-e2e-test-records.md#run-mp-installed-app-smoke-wrapper-20260716-1613`. Prior records: `docs/live-e2e-test-records.md#run-mp-installed-persistence-autostart-gate-20260716-1552`, `docs/live-e2e-test-records.md#run-mp-installed-v0520-nsis-state-and-persistence-20260716-1548`. |
| Installed app window visual smoke | Partial | Latest Computer Use window smoke at 2026-07-16 11:25-11:32 +08:00 opened the installed `v0.5.20` app, confirmed New Chat avatar/prompt state, Review Queue cards, Pipelines credential-warning fallback, and Activity checkpoint loading recovery. It found one installed-build issue: Pull Requests can still show raw `/project-links/.../pull-requests HTTP 401` text when Azure credentials are expired. Current source now has direct no-raw-route regression proof for the PR list and PR insight artifact list/lookup/save paths: PR API auth-error tests passed 4/4, the focused Pull Requests browser failure test passed, full route-cache passed 21/21, and desktop typecheck passed. Rebuild/install a new package before claiming the installed app is fixed. Records: `docs/live-e2e-test-records.md#run-mp-pr-auth-error-no-raw-route-20260716-1137`, `docs/live-e2e-test-records.md#run-mp-installed-window-visual-smoke-20260716-1132`. |
...
7. Close the remaining installed-package proof gap for the current package version. Program Files still contains a healthy `0.5.20` NSIS-shaped install, while the current local package is `0.5.22`. The `0.5.22` MSI payload, MSI internal metadata, NSIS metadata, desktop executable metadata, package smoke, packaged vision smoke, and extracted-MSI stale-template scan are proven without installing. For MSI release acceptance, run `.\scripts\windows\install-and-verify-msi-state.ps1 -ExpectedVersion 0.5.22 -MsiPath .\apps\desktop\src-tauri\target\release\bundle\msi\MergePilot_0.5.22_x64_en-US.msi` from elevated PowerShell. This path closes running old MergePilot processes before install and then runs the strict installed parity, persistence, safety, and vision gates.
