...
| Packaged installer behavior | Partially | Packaged sidecar native runtime smoke, release desktop shell first-message API smoke, local MSI payload extraction smoke, version-bumped `0.5.11` MSI live vision smoke, and the published `v0.5.11` through `v0.5.20` MSI payload/live-vision smokes all pass. Latest installed reprobe confirms the running Program Files daemon is now `0.5.20`, auth/avatar pass, and installed restart persistence passes. Remaining gap: the current Program Files desktop is the NSIS install shape (`__TAURI_BUNDLE_TYPE_VAR_NSS` plus `uninstall.exe`), not MSI-strict parity. Install `MergePilot_0.5.20_x64_en-US.msi` specifically from elevated PowerShell before claiming MSI payload parity. Native click/pixel automation is also not yet reliable. |
...
| Playwright test discovery | Pass | Current default browser E2E run at 2026-07-07 01:04-01:06 +08:00 discovered 84 Chromium tests across `tests/e2e/chat-layout.spec.ts`, `tests/e2e/live-app-business.spec.ts`, `tests/e2e/review-queue.spec.ts`, and `tests/e2e/settings-permissions.spec.ts`; 54 mocked/default tests passed and 30 live-app tests skipped by design. The mocked release smoke selector `--grep "@smoke"` covers 9 release-critical browser workflows and also passed 9/9 at 2026-07-07 01:03 +08:00. Record: `docs/live-e2e-test-records.md#run-mp-post-v0519-browser-gates-20260707-0103`. |
...
| Add installed-build UI smoke protocol. | Protocol exists and most installed runtime checks pass: installed desktop shell starts the bundled daemon from `C:\Program Files\MergePilot`, installed `/healthz` loads user config and Azure OpenAI `gpt-4o`, auth APIs expose the signed-in user/avatar data, user visual review confirms the footer avatar renders, temp repo/index/chat workflow routes work through the installed daemon, and legacy `C:\Program Files\CICD-Agent` cleanup passes. Latest 2026-07-16 reprobe confirms the installed daemon is `0.5.20`, daemon hash matches the `v0.5.20` MSI payload, auth/avatar pass, and restart persistence passes after daemon restart. Strict MSI parity remains partial because the installed desktop is the NSIS bundle shape (`__TAURI_BUNDLE_TYPE_VAR_NSS`) and `C:\Program Files\MergePilot\uninstall.exe` remains; install the MSI asset specifically before claiming MSI parity. The 09:40-09:47 Computer Use retest confirmed the local plugin-cache repair restores bootstrap, app discovery, and accessibility proof for the installed MergePilot window, including navigation, user, and ClaimBot_API Project Link state. Native screenshot/click proof still fails: both app-id and process-backed captures show unrelated river/Windows Spotlight imagery, and activation plus a low-risk Settings click fails with `failed to activate captured window`. |
...
Most broad live/release gate records below were collected around 2026-07-06 to 2026-07-07. The latest source and mocked route-cache stabilization gate was collected on 2026-07-16 00:56-01:00 +08:00.

| Gate | Status | Evidence |
|---|---|---|
| Workspace UX source and route-cache stabilization | Pass | Latest rerun at 2026-07-16 00:56-01:00 +08:00: root lint passed after adding ESLint 9 flat config, root package typecheck/test passed, desktop typecheck/test/build passed, `git diff --check` passed with only CRLF warnings, and `.\scripts\windows\pnpm-project.ps1 exec playwright test tests/e2e/route-cache.spec.ts --project=chromium` passed 20/20. This covers New Chat prompt stability, warm route switching, stale Project Link switching, PR/Review Queue/Pipelines cache behavior, Activity cache behavior, and collapsed raw output. Record: `docs/live-e2e-test-records.md#run-mp-workspace-ux-source-gate-20260716-0100`. |
...
| Default Chromium browser gate | Pass | Latest broad mocked browser rerun at 2026-07-16 01:20-01:25 +08:00 passed `review-queue.spec.ts` 3/3, `settings-permissions.spec.ts` 1/1, and full `chat-layout.spec.ts` 50/50. Earlier default Chromium run at 2026-07-06 passed 54/54 non-live tests and skipped 30 gated live-app tests by design. Installed daemon reprobe now reports version `0.5.20`, Azure OpenAI `gpt-4o`, `cloudSecrets: false`, and `cloudSessions: true`; MSI desktop parity remains separate. Records: `docs/live-e2e-test-records.md#run-mp-workspace-ux-browser-gate-20260716-0125` and `docs/live-e2e-test-records.md#run-mp-installed-v0520-reprobe-20260716-0113`. |
...
| Live ADO discovery/pipeline/PR insight | Pass | Latest rerun at 2026-07-07 02:04-02:05 +08:00 after `v0.5.20`: `MERGEPILOT_E2E_LIVE_ADO=1 .\scripts\windows\pnpm-project.ps1 --filter @mergepilot/core test -- test/liveAdoDiscovery.test.ts test/liveAdoPipeline.test.ts` passed 3/3 with the destructive queue case skipped; `MERGEPILOT_E2E_LIVE_ADO=1 .\scripts\windows\pnpm-project.ps1 --filter @mergepilot/daemon test -- test/liveAdoPrInsight.test.ts` passed 1/1 against a real ClaimBot_API PR insight workflow. ClaimBot_API pipeline `#117` remains the live pipeline target, latest run stayed `4680 / 20260706.1`, and no ADO mutation occurred. Record: `docs/live-e2e-test-records.md#run-mp-post-v0520-readonly-business-gates-20260707-0204`. |
...
| Mocked browser release smoke | Pass | Latest rerun at 2026-07-07 02:05 +08:00 after the `v0.5.20` release publication: `.\scripts\windows\pnpm-project.ps1 exec playwright test --project=chromium --grep "@smoke"` passed 9/9 in 34.2 seconds. The smoke set is intentionally mocked and non-mutating; it covers Chat shell, PR insight controls, pipeline controls, missing-pipeline setup, natural-language PR/pipeline routing, architecture/source references, Review Queue acknowledgement, and Settings Key Vault/local-env permission handling. Record: `docs/live-e2e-test-records.md#run-mp-post-v0520-readonly-business-gates-20260707-0204`. |
...
| Full live app business gate | Pass | Latest rerun at 2026-07-07 01:08-01:15 +08:00: `MERGEPILOT_E2E_LIVE_APP=1 MERGEPILOT_E2E_LIVE_ADO=1 .\scripts\windows\pnpm-project.ps1 exec playwright test tests/e2e/live-app-business.spec.ts --project=chromium` passed 30/30 in 6.4 minutes with `MERGEPILOT_E2E_DESTRUCTIVE` unset. The full run covered selected staging, pending approval reload/restore, approval denial and feedback replanning, stage-and-commit, commit validation failure, empty commit guard, staged-only summaries, draft commit messages, remote credential redaction, secret-like diff redaction, dirty branch switching, target merge, merge/rebase/pull/stash recovery, restore/revert/tag workflows, single-tag publication, ClaimBot_API pipeline #117 discovery-to-save, read-only failure inspection, rerun approval preparation, and direct trigger approval preparation. No `%TEMP%\mergepilot-live-*`, `%TEMP%\mergepilot-daemon-live-pr-insight-*`, or `%TEMP%\mp-installed-*` directories remained, `/project-links` retained only the expected long-lived links, and ADO pipeline `#117` latest run remained `4680`. Record: `docs/live-e2e-test-records.md#run-mp-live-app-business-full-post-v0519-20260707-0108`. |
...
| Packaged MSI payload smoke | Pass | Latest retests prove both basic packaged runtime health and live vision streaming. The 2026-07-06 03:57 packaged MSI smoke returned `ok: true`, `legacyCleanupWixValidated: true`, `healthVersion: 0.5.10`, `refreshFilesSeen: 1`, `refreshFilesIndexed: 1`, `workflowPhase: inspect_environment`, and `chatStatus: 200`. The 2026-07-06 08:58-09:02 packaged live vision smoke built a fresh `0.5.10` MSI and verified the extracted daemon recognized `MP VISION TEST`, blue square, and red circle with `leaksControlJson: false` and `duplicateSentence: false`. The published `v0.5.11` through `v0.5.19` MSI assets all passed payload and live vision smokes. Latest 2026-07-07 00:42-00:56 +08:00 validation downloaded the published GitHub Release `MergePilot_0.5.19_x64_en-US.msi`, matched release SHA256 `EE62A85D7D19C8C011BA97B1307D02786581A5E8A8288BF8ECED022FFA613926`, passed extracted payload smoke with `healthVersion: 0.5.19`, and passed live vision with final answer mentioning `MP VISION TEST`, blue square, and red circle, `leaksControlJson: false`, and `duplicateSentence: false`. Records: `docs/live-e2e-test-records.md#run-mp-packaged-installed-smoke-rerun-20260706-0357`, `docs/live-e2e-test-records.md#run-mp-packaged-live-vision-stream-20260706-0858`, `docs/live-e2e-test-records.md#run-mp-version-bumped-msi-payload-20260706-0913`, `docs/live-e2e-test-records.md#run-mp-github-release-msi-payload-smoke-20260706-1951`, `docs/live-e2e-test-records.md#run-mp-github-release-msi-live-vision-20260706-2007`, `docs/live-e2e-test-records.md#run-mp-github-release-v0512-acceptance-20260706-2037`, `docs/live-e2e-test-records.md#run-mp-github-release-v0513-acceptance-20260706-2113`, `docs/live-e2e-test-records.md#run-mp-github-release-v0514-acceptance-20260706-2148`, `docs/live-e2e-test-records.md#run-mp-github-release-v0515-acceptance-20260706-2219`, `docs/live-e2e-test-records.md#run-mp-github-release-v0516-acceptance-20260706-2258`, `docs/live-e2e-test-records.md#run-mp-github-release-v0517-acceptance-20260706-2340`, `docs/live-e2e-test-records.md#run-mp-github-release-v0518-acceptance-20260707-0027`, `docs/live-e2e-test-records.md#run-mp-github-release-v0519-acceptance-20260707-0056`. |
| Fresh installed app first-run smoke | Partial | Latest installed-state verifier at 2026-07-16 compared current `C:\Program Files\MergePilot` against `MergePilot_0.5.20_x64_en-US.msi`. Installed `/healthz` is healthy at version `0.5.20`, authenticated as `Zhou Ping`, avatar data is present, daemon hash matches the MSI payload, legacy `C:\Program Files\CICD-Agent` and old publisher shortcut folders are absent, and the current shortcut exists. Strict MSI desktop parity still fails because the installed desktop is the NSIS bundle shape (`__TAURI_BUNDLE_TYPE_VAR_NSS`) while the MSI payload is `__TAURI_BUNDLE_TYPE_VAR_MSI`; `uninstall.exe` is present and versioned `0.5.20`. Computer Use bootstrap/app discovery/accessibility proof from 09:40 still passes for the native MergePilot window, but activation/click proof remains unreliable. Records: `docs/live-e2e-test-records.md#run-mp-installed-v0520-reprobe-20260716-0113`, `docs/live-e2e-test-records.md#run-mp-installed-computer-use-recheck-20260706-0940`. |
| Installed restart persistence smoke | Pass | Latest retest at 2026-07-16 01:13 +08:00: `.\scripts\windows\installed-restart-persistence-smoke.ps1 -ExpectedVersion 0.5.20` verified installed daemon restart persistence at completion level. The script created temporary Project Link `9987330225d77d17`, chat session `chat_1784135608476_f14637`, observed terminal SSE `done`, verified assistant completion `persistence-ok-mp-installed-persist-20260716-011326` before and after restarting `C:\Program Files\MergePilot\mergepilot-daemon.exe`, then deleted the chat session, Project Link, and temp repo. Record: `docs/live-e2e-test-records.md#run-mp-installed-v0520-reprobe-20260716-0113`. |
...
7. Install the published `0.5.19` package as administrator by running `.\scripts\windows\install-and-verify-msi-state.ps1 -ExpectedVersion 0.5.19 -MsiPath output\live-e2e\release-v0.5.19\MergePilot_0.5.19_x64_en-US.msi` from elevated PowerShell; the script wraps MSI install, strict hash parity, daemon health/auth/avatar, and installed Program Files live vision smoke. The `0.5.19` MSI payload itself is proven healthy, and the non-elevated guard fails cleanly with `requiresElevation: true` instead of attempting a system install.
