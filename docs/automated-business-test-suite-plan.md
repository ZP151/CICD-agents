...
| Packaged installer behavior | Partially | Packaged sidecar native runtime smoke, release desktop shell first-message API smoke, local MSI payload extraction smoke, version-bumped `0.5.11` MSI live vision smoke, the published `v0.5.11`, `v0.5.12`, `v0.5.13`, `v0.5.14`, `v0.5.15`, `v0.5.16`, `v0.5.17`, `v0.5.18`, and `v0.5.19` MSI payload/live-vision smokes all pass. Installed app runtime/auth/avatar, Start Menu shortcut, and legacy cleanup pass for the currently installed app, but Program Files still contains stale `0.5.10` binaries. Remaining gap: install the `0.5.19` MSI as administrator and prove strict installed payload parity plus installed live vision streaming. Native click/pixel automation is also not yet reliable. |
...
| Live ADO discovery/pipeline/PR insight | Pass | Latest rerun at 2026-07-07 01:22-01:23 +08:00: `MERGEPILOT_E2E_LIVE_ADO=1 .\scripts\windows\pnpm-project.ps1 --filter @mergepilot/core test -- test/liveAdoDiscovery.test.ts test/liveAdoPipeline.test.ts` passed 3/3 with the destructive queue case skipped; `MERGEPILOT_E2E_LIVE_ADO=1 .\scripts\windows\pnpm-project.ps1 --filter @mergepilot/daemon test -- test/liveAdoPrInsight.test.ts` passed 1/1 against a real ClaimBot_API PR insight workflow. ClaimBot_API pipeline `#117` remains the live pipeline target, latest run stayed `4680 / 20260706.1`, and no ADO mutation occurred. Follow-up installed daemon probe stayed healthy and no business temp directories remained. Record: `docs/live-e2e-test-records.md#run-mp-live-ado-readonly-post-v0519-20260707-0122`. |
...
| Mocked browser release smoke | Pass | Latest rerun at 2026-07-07 01:03 +08:00 after the `v0.5.19` release publication and Playwright local-toolchain correction: `.\scripts\windows\pnpm-project.ps1 exec playwright test --project=chromium --grep "@smoke"` passed 9/9 in 24.8 seconds. The smoke set is intentionally mocked and non-mutating; it covers Chat shell, PR insight controls, pipeline controls, missing-pipeline setup, natural-language PR/pipeline routing, architecture/source references, Review Queue acknowledgement, and Settings Key Vault/local-env permission handling. Record: `docs/live-e2e-test-records.md#run-mp-post-v0519-browser-gates-20260707-0103`. |
| GitHub CI and release publication | Pass | Latest publication: commit `517ad76` passed GitHub CI run `28807757070` on `main` across Node 22 Ubuntu/Windows and Desktop Tauri macOS/Windows jobs. Tag `v0.5.19` passed Release run `28807760738`, including Windows installer, macOS installer, and GitHub Release jobs. Published assets are `MergePilot_0.5.19_x64_en-US.msi`, `MergePilot_0.5.19_x64-setup.exe`, and `MergePilot_0.5.19_aarch64.dmg`; the MSI digest is `sha256:ee62a85d7d19c8c011ba97b1307d02786581a5e8a8288bf8eced022ffa613926`. Record: `docs/live-e2e-test-records.md#run-mp-github-release-v0519-acceptance-20260707-0056`. |
...
| Fresh installed app first-run smoke | Partial | Latest installed-state verifier at 2026-07-07 00:56 +08:00 compared current `C:\Program Files\MergePilot` against the published GitHub Release `MergePilot_0.5.19_x64_en-US.msi`. Installed `/healthz` is healthy, authenticated as `Zhou Ping`, avatar data is present, legacy `C:\Program Files\CICD-Agent` and old publisher shortcut folders are absent, and the current shortcut exists. Strict release parity still fails because installed Program Files remains `0.5.10`; the uninstall entry is `MergePilot 0.5.10`, and installed desktop/daemon hashes do not match the `v0.5.19` release MSI payload. Computer Use bootstrap/app discovery/accessibility proof from 09:40 still passes for the native MergePilot window, but activation/click proof remains unreliable. Records: `docs/live-e2e-test-records.md#run-mp-github-release-v0519-acceptance-20260707-0056`, `docs/live-e2e-test-records.md#run-mp-installed-computer-use-recheck-20260706-0940`. |
