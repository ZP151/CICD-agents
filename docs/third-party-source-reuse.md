...
| 2026-08-03 | `react-easy-crop` | `^6.2.3` | MIT | `apps/desktop` | MP-013: pre-send crop/zoom/rotate for composer image attachments. | React >=16.4 peer ✓. Used only inside `ImageEditModal`; conversion runs on a local canvas copy before send and never modifies the source. Not extended into a full image workbench. |
