...
const GATES = [
  ...
  { id: "browser-e2e-mocked", tier: "mocked-browser-e2e", required: true, requireNoSkips: true, timeoutMs: 2_700_000, cmd: `powershell -NoProfile -ExecutionPolicy Bypass -File "${path.join(repoRoot, "scripts", "windows", "run-mocked-browser-e2e.ps1")}" -TestPath tests/e2e/warmup.spec.ts -Workers 1 && powershell -NoProfile -ExecutionPolicy Bypass -File "${path.join(repoRoot, "scripts", "windows", "run-mocked-browser-e2e.ps1")}" -TestPath tests/e2e/chat-layout.spec.ts -Workers 1 && powershell -NoProfile -ExecutionPolicy Bypass -File "${path.join(repoRoot, "scripts", "windows", "run-mocked-browser-e2e.ps1")}" -TestPath tests/e2e/settings-permissions.spec.ts -Workers 1 && powershell -NoProfile -ExecutionPolicy Bypass -File "${path.join(repoRoot, "scripts", "windows", "run-mocked-browser-e2e.ps1")}" -TestPath tests/e2e/route-cache.spec.ts -Workers 1`, description: "mocked browser E2E (warmup, chat-layout, settings-permissions, route-cache)" },
  { id: "browser-e2e-source-live", tier: "source-live-e2e", required: true, requireNoSkips: true, timeoutMs: 1_800_000, cmd: `powershell -NoProfile -ExecutionPolicy Bypass -File "${path.join(repoRoot, "scripts", "windows", "run-live-app-e2e.ps1")}" -LiveAdo -RestartMismatchedDaemon`, description: "source Vite + daemon live E2E (real ADO reads)" },
