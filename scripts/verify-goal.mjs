...
const GATES = [
  ...
  { id: "browser-e2e-mocked", tier: "mocked-browser-e2e", required: true, requireNoSkips: true, timeoutMs: 2_700_000, cmd: `powershell -NoProfile -ExecutionPolicy Bypass -File "${path.join(repoRoot, "scripts", "windows", "run-mocked-browser-e2e.ps1")}" -TestPath tests/e2e/warmup.spec.ts && powershell -NoProfile -ExecutionPolicy Bypass -File "${path.join(repoRoot, "scripts", "windows", "run-mocked-browser-e2e.ps1")}" -TestPath tests/e2e/chat-layout.spec.ts && powershell -NoProfile -ExecutionPolicy Bypass -File "${path.join(repoRoot, "scripts", "windows", "run-mocked-browser-e2e.ps1")}" -TestPath tests/e2e/settings-permissions.spec.ts && powershell -NoProfile -ExecutionPolicy Bypass -File "${path.join(repoRoot, "scripts", "windows", "run-mocked-browser-e2e.ps1")}" -TestPath tests/e2e/route-cache.spec.ts`, description: "mocked browser E2E (warmup, chat-layout, settings-permissions, route-cache)" },
