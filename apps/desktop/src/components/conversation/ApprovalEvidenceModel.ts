...
export function approvalPreflightRows(preflight?: ApprovalPreflightEvidence): Array<{ label: string; value: string }> {
...
export function toolCommandPreview(toolName?: string, args?: Record<string, unknown>): string {
  ...
  const summary = conciseArgSummary(args);
  return summary ? `${toolName} ${summary}` : toolName;
}

export function workflowBoundaryText(workflow: ApprovalWorkflowEvidence, nextHint?: string): string {
...
function validationCommandSourceLabel(source: Extract<ApprovalPreflightEvidence, { kind: "validation" }>["commandSource"]): string {
  ...
  if (source === "project_link") return "Project Link";
  return source;
}
...
function conciseArgSummary(args?: Record<string, unknown>): string {
  if (!args) return "";
  return Object.entries(args)
