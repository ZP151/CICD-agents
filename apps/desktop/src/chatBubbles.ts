...
export function finaliseAssistantResponseBubbles<T extends ChatBubbleModel>(
  ...
  if (matchingAssistantIndex >= 0) {
    ...
    const text = finalText || streamText || sanitizeAssistantText(bubble.text) || sanitizeAssistantText(partText);
    const parts = finalText
      ? conversationPartsFromAssistantBubble({ text, meta: mergedMeta })
      : conversationPartsFromAssistantBubble({ text: bubble.text, parts: bubble.parts, meta: mergedMeta });
...
function findMatchingAssistantBubbleIndex<T extends ChatBubbleModel>(bubbles: T[], candidates: string[]): number {
  ...
  for (let index = bubbles.length - 1; index >= 0; index -= 1) {
    ...
    if (candidates.some((candidate) => texts.some((text) => assistantTextMatches(text, candidate)))) return index;
  }
  return -1;
}
