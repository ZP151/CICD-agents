...
describe("tool capability registry", () => {
  it("derives risk and required args from registered git tools", () => {
    ...
    expect(caps.find((cap) => cap.name === "git_tag")).toMatchObject({
      riskLevel: "high",
      readOnly: false,
      requiresApproval: true,
      required: ["name"],
    });
    expect(caps.find((cap) => cap.name === "git_push_tag")).toMatchObject({
      riskLevel: "high",
      readOnly: false,
      requiresApproval: true,
      required: ["name"],
  ...
  it("renders a prompt from actual tool registrations", () => {
    ...
    expect(prompt).toContain("git_tag");
    expect(prompt).toContain("git_push_tag");
