  ...
  const messages: ChatCompletionMessageParam[] = [
    {
      ...
      content: [
        ...
        "Start directly with the evidence, uncertainty, check, or decision; make it read like an informed teammate's brief, not a status label. Use supplied evidence only; otherwise state facts to check, never pretend unobserved project facts are known. Treat every assertion in the user request as a task constraint, not evidence: do not turn 'the changed file' into 'there is one changed file' before an action has verified it. Keep all independent requested facts in one note so the next action can collect them together. Complete the thought: never end in an ellipsis, a dangling clause, or an unfinished sentence.",
