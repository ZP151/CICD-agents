...
export const CHAT_AGENT_USE_CASES: ChatAgentUseCase[] = [
  ...
  {
    id: "pr-creation",
    title: "Create and update pull requests",
    intentSignals: ["create pr", "open pull request", "push and create pr", "link work item", "update pr", "change pr title", "edit pr description", "add reviewer", "remove reviewer", "add tag", "add label"],
    readTools: ["git_status", "git_current_branch", "git_log", "git_remote"],
    writeTools: ["git_push", "ado_create_pr", "ado_update_pull_request", "ado_add_pull_request_reviewer", "ado_remove_pull_request_reviewer", "ado_add_pull_request_label", "ado_remove_pull_request_label", "ado_link_work_item"],
    approval: "required",
    expectedBehavior: "Only mutate a PR when requested, generate exact typed proposals for title/description/status/reviewer/label/work-item changes, and keep required-reviewer/policy configuration as a separate high-risk policy operation.",
  },
  {
  ...
  },
  {
    ...
    readTools: ["ado_pipelines_get_build_definitions", "ado_pipelines_get_builds", "ado_list_pipeline_runs", "ado_pipelines_get_run", "ado_get_build_timeline", "ado_get_build_log_excerpt"],
